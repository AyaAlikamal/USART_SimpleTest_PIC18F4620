...
void usart_init(){
    ...
    usart_obj.tx_cfg.usart_tx_enable = 1;
    usart_obj.tx_cfg.usart_tx_interrupt_enable = 0 ;
    usart_obj.tx_cfg.usart_tx_9bit_enable = 0 ;
    usart_obj.rx_cfg.usart_rx_enable = 1;
    usart_obj.rx_cfg.usart_rx_interrupt_enable = 0 ;
    usart_obj.rx_cfg.usart_rx_9bit_enable = 0 ;
    usart_obj.EUSART_Framming_Error_HUNDELER = ((void*)0);
    usart_obj.EUSART_Overrun_Error_HUNDELER = ((void*)0);
    usart_obj.EUSART_RX_DEFUALTINTERRUPT_HUNDELER = ((void*)0);
    usart_obj.EUSART_TX_DEFUALTINTERRUPT_HUNDELER = ((void*)0);
...
while(1){
# 45 "application.c"
    ret = USART_ASYNC_ReadByte_Non_Blocking(&rec_usart);
    if(ret == (Std_ReturnType)0x01){
        if(rec_usart == 'a'){
        led_turn_on(&led1);
        ret = USART_ASYNC_WriteString_Blocking("LED on\r", 7);
        }
        else if(rec_usart == 'b'){
        led_turn_off(&led1);
         ret = USART_ASYNC_WriteString_Blocking("LED off\r", 8);
        }
    }
# 66 "application.c"
}
